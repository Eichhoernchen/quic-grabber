...
func DialAddrFromAddr(addr string, saddr string, tlsConf *tls.Config, config *quic.Config) (quic.Session, error, *net.UDPConn) {
	...
	if runtime.GOOS == "linux" {
		raw_conn, err := udpConn.SyscallConn()
		if err == nil {
			raw_conn.Control(func(fd uintptr) {
				...
				if udpAddr.IP.To4() == nil {
					...
					err = syscall.Connect(intfd, &sa)
					if err != nil {
						log.Println("Failed connecting IPv6 socket")
					}
				} else {
					...
					err = syscall.Connect(intfd, &sa)
					if err != nil {
						log.Println("Failed connecting IPv4 socket")
					}
				}
...
func scan_quic(addr string, sni string, original map[string]interface{}, result_chan chan<- *myTrace) {
	...
	go func(t *myTrace) {
		session, err, sock := DialAddrFromAddr(t.addr, *src_ip, &tls.Config{InsecureSkipVerify: true, ServerName: t.sni}, &quic.Config{Versions: []quic.VersionNumber{0x51303433, 0x51303339}, HandshakeTimeout: 5 * time.Second, IdleTimeout: 5 * time.Second, QuicTracer: qtrace.Tracer{ClientGotHandshakeMsg: t.ClientGotHandshakeMsg}})
		//		session, err, sock := DialAddrFromAddr(t.addr, *src_ip, &tls.Config{InsecureSkipVerify: true, ServerName: t.sni}, &quic.Config{Versions: quic.SupportedVersions, HandshakeTimeout: 5 * time.Second, IdleTimeout: 5 * time.Second, QuicTracer: qtrace.Tracer{ClientGotHandshakeMsg: t.ClientGotHandshakeMsg}})
...
func main() {
	...
	result_chan := make(chan *myTrace, *num_concurrent)

	stdin_chan := make(chan string, 1)
	running_chan := make(chan bool, *num_concurrent)
	//log.Printf("Allowing %d parallel attempts\n", cap(running_chan))
	running := true
